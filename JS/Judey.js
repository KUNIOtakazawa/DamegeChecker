...
class Judey {
    ...
    constructor(Selector, SearchType, ans) {
        ...
        this.RetText = (PropartyName) => {
            ...
            const Ret = (elem) => {
                if (PropartyName.includes("dataset.")) {
                    return elem.dataset[PropartyName.replaceAll("dataset.", "")];
                }
                else if (PropartyName.includes("data-")) {
                    return elem.dataset[PropartyName.replaceAll("data-.", "")];
                }
                else return elem[PropartyName];
            }
            let e = [];
            this.EachElem(elem => e.push(Ret(elem)));;
        ...
        this.RemEvent = (e, act) => {
            ...
            this.EachElem(elem => {
                ans.forEach(Event => {
                    ...
                    elem.removeEventListener(Event, act, { capture: false });
        ...
        this.InpText = (value = null) => {
            if (this._Length < 0) return this;
            const NoneValue = [undefined, null].includes(value);
            ...
            if (this._Length === 0) {
                ...
                const Result = [];
                this.EachElem((elem, i) => {
                    if (NoneValue) Result.push(elem.value);
                    else {
                        if (typeof value !== "object")
                            elem.value = value;
                        else
                ...
                return !NoneValue ? this : Result.length === 1 ? Result[0] : Result;
            }
        };
        return this;
    }
